...
const loadParts = async () => {
  const response = await fetch('/src/components.json');
...
const createParametersMap = () => {
  parameters.value.forEach(param => {
    parametersMap.value[param.name] = param.value;
  });
};

const replaceParameters = (str: string) => {
  const regex = /\$\{(\w+)\}/g;
  return str.replace(regex, (_, key) => parametersMap.value[key]?.toString() || '0');
};

const computeDimensions = (partName: string): [number, number, number] => {
  const part = partsConfig.value[partName];
  return [
    parseFloat(replaceParameters(part.length)),
    parseFloat(replaceParameters(part.width)),
    parseFloat(replaceParameters(part.thickness))
  ];
};
